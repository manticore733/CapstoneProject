...
export class ClientDetails {
  ...
  // NEW METHOD: Check if at least one document is uploaded
  ...
  // Modal methods
  openApproveModal(): void {
    // NEW: Check for documents first
    if (!this.hasDocuments()) {
      alert('⚠️ Client must upload at least one document before approval.');
      return;
    }
    this.showApproveModal = true;
    this.initialBalance = 0;
  ...
  openRejectModal(): void {
    // NEW: Check for documents first
    if (!this.hasDocuments()) {
      alert('⚠️ Client must upload at least one document before rejection.');
      return;
    }
    this.showRejectModal = true;
    this.remark = '';
